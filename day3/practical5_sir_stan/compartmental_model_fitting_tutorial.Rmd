...
We now need to change our model to fit the incidence from our model to the data rather than the prevalence. First we need to calculate the incidence, which is equal to the number of people leaving the exposed component each day.  We can calculate this from our compartmental model by considering the daily change in cumulative incidence.

The cumulative incidence, $C$ can be calculated by adding an additional differential equation to our model block.
$$
\frac{dC}{dt} = \gamma E.
$$
Then the incidence can be calculated by integrating this equation over each day
$$
C = \int_{t-1}^{t}\gamma E dt.
$$
You can assume that $t_0$ is equal to zero.

Q6) Make a new copy of our SEIR model by re-saving ```seir_prevalence.stan``` as ```seir_incidence.stan```. The incidence can be calculated by first calculating the adding in the differential equation and then changing the transformed parameters block as follows (note only the for loop around the incidence should have changed). You will also need to add an initial condition for this parameter.
...
smr_y <- as.data.frame(summary(fit_sir_negbin,
                               pars = params, probs = c(0.05, 0.5, 0.95))$summary)
colnames(smr_y) <- make.names(colnames(smr_y)) # to remove % in the col names
dat2 = data.frame(t = 1:13)
dat2$infected = c(infected[-1])
print(ggplot(smr_y, mapping = aes(x = t)) +
  geom_ribbon(aes(ymin = X5., ymax = X95.), fill = c_posterior, alpha = 0.35) +
  geom_line(mapping = aes(x = t, y = X50.), color = c_posterior) +
  geom_point(data = dat2, mapping = aes(y = infected)) +
